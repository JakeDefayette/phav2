...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!practiceId) {
      console.warn('Could not determine practice ID for webhook event:', event.data.email_id);
      return NextResponse.json(
    ...
    const trackingEvent = await emailTrackingService.processWebhookEvent(event, practiceId);

    if (trackingEvent) {
      console.log(`Successfully processed ${event.type} event for ${event.data.email_id}`);
 ...
 */
async function extractPracticeId(event: ResendWebhookEvent): Promise<string | null> {
  // Method 1: Look for practice_id in email tags
  if (event.data.tags) {
    const practiceIdTag = event.data.tags.find(tag => tag.name === 'practice_id');
    if (practiceIdTag) {
...
async function processAdvancedEvents(
  ...
  try {
    if (event.type === 'email.bounced') {
      ...
      const bounceAnalysis = await emailBounceHandler.processBounce(event, trackingEvent);
      console.log(`Bounce analysis: ${bounceAnalysis.classification} - ${bounceAnalysis.action} - ${bounceAnalysis.reason}`);
    }
    
    if (event.type === 'email.complained') {
      // Use advanced complaint processing
      const complaintAnalysis = await emailBounceHandler.processComplaint(event, trackingEvent);
      console.log(`Complaint analysis: ${complaintAnalysis.severity} - ${complaintAnalysis.action} - ${complaintAnalysis.reason}`);
    }
  } catch (error) {
...
async function handleCriticalEvents(
  ...
  try {
    ...
    if (event.type === 'email.bounced' || event.type === 'email.complained') {
      ...
      console.log(`Critical email event: ${event.type} for ${trackingEvent.recipientEmail}`);
      ...
      const notification = {
        type: event.type === 'email.bounced' ? 'EMAIL_BOUNCE' : 'EMAIL_COMPLAINT',
        practiceId: trackingEvent.practiceId,
        recipientEmail: trackingEvent.recipientEmail,
        emailId: trackingEvent.emailId,
        reason: trackingEvent.bounceReason || trackingEvent.complaintFeedbackType,
...
type EmailTrackingEvent = import('@/shared/services/email/types').EmailTrackingEvent; 