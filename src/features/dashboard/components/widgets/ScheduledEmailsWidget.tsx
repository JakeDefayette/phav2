...
export function ScheduledEmailsWidget({
  ...
  const handleCancelEmail = async (scheduledEmailId: string) => {
    try {
      const result = await emailService.cancelScheduledEmail(
        scheduledEmailId,
        practiceId
      );
            ...
            >
              <div className='flex justify-between items-start mb-2'>
                ...
                <div className='flex items-center space-x-2'>
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${getStatusColor(email.status)}`}
                  >
                    {email.status}
                  </span>
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${getPriorityColor(email.priority)}`}
                  >
                    {email.priority}
                  </span>
                </div>
              ...
              <div className='flex justify-between items-center text-sm text-gray-500'>
                <div className='flex items-center space-x-4'>
                  <span>Type: {email.templateType.replace('_', ' ')}</span>
                  <span>
...
export default ScheduledEmailsWidget;
