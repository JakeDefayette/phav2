...
import { AlertTriangle, TrendingUp, TrendingDown, Mail, Shield, AlertCircle } from 'lucide-react';
...
export function EmailDeliverabilityWidget({ 
  ...
  const fetchStats = async () => {
    try {
      setError(null);
      const response = await fetch(`/api/email/deliverability-stats?practiceId=${practiceId}&hoursBack=${hoursBack}`);
  ...
  const getRateStatus = (rate: number, thresholds: { good: number; warning: number }) => {
    ...
    return (
      <Card className="p-6">
        <div className="flex items-center space-x-2 text-red-600">
          ...
          <span className="text-sm font-medium">Error loading deliverability stats</span>
        </div>
  ...
  const complaintStatus = getRateStatus(stats.complaintRate, { good: 0.05, warning: 0.1 });
  const deliveryStatus = getRateStatus(100 - stats.deliveryRate, { good: 2, warning: 5 });

  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Email Deliverability</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          <div className="mt-2 text-xs text-gray-500">
            {formatNumber(stats.totalBounced)} of {formatNumber(stats.totalSent)} emails
          </div>
        </div>
        ...
        <div className="bg-white border rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <complaintStatus.icon className={`h-5 w-5 ${complaintStatus.color}`} />
          </div>
          <div className="mt-2 text-xs text-gray-500">
        ...
        <div className="bg-white border rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${deliveryStatus.status === 'good' ? 'text-green-600' : deliveryStatus.color}`}>
                {formatPercentage(stats.deliveryRate)}
              </p>
            </div>
            <Mail className={`h-5 w-5 ${deliveryStatus.status === 'good' ? 'text-green-600' : deliveryStatus.color}`} />
          </div>
          <div className="mt-2 text-xs text-gray-500">
      ...
      {/* Bounce Breakdown */}
      <div className="mb-6">
        <h4 className="text-sm font-medium text-gray-700 mb-3">Bounce Breakdown</h4>
        <div className="grid grid-cols-2 gap-4">
