...
describe('EmailTemplateService', () => {
  ...
  describe('utility methods', () => {
    ...
    it('should check if template exists', () => {
      expect(EmailTemplateService.hasTemplate(EmailTemplateType.REPORT_DELIVERY)).toBe(true);
      expect(EmailTemplateService.hasTemplate(EmailTemplateType.REPORT_READY)).toBe(true);
      expect(EmailTemplateService.hasTemplate('non_existent' as EmailTemplateType)).toBe(false);
    });

    it('should get template info', () => {
      const info = EmailTemplateService.getTemplateInfo(EmailTemplateType.REPORT_DELIVERY);
    ...
    it('should return null for non-existent template info', () => {
      const info = EmailTemplateService.getTemplateInfo('non_existent' as EmailTemplateType);
    ...
    it('should test template with sample data', async () => {
      ...
      const result = await EmailTemplateService.testTemplate(EmailTemplateType.REPORT_DELIVERY);
  ...
  describe('custom templates', () => {
    it('should register and clear custom templates', () => {
      const customTemplate = {
        component: ({ name }: any) => React.createElement('div', {}, `Custom ${name}`),
      ...
      EmailTemplateService.registerTemplate('custom_test' as EmailTemplateType, customTemplate);

      // Check it was registered
      expect(EmailTemplateService.hasTemplate('custom_test' as EmailTemplateType)).toBe(true);
      expect(EmailTemplateService.getCustomTemplates()).toContain('custom_test');

      // Clear custom templates
      EmailTemplateService.clearCustomTemplates();

      // Check it was cleared
      expect(EmailTemplateService.hasTemplate('custom_test' as EmailTemplateType)).toBe(false);
