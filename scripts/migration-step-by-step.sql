-- Step-by-Step Migration: Create enums first, then tables
-- Description: Separated migration to ensure enums are created before tables
...
-- User roles
CREATE TYPE IF NOT EXISTS user_role_enum AS ENUM ('parent', 'practitioner', 'admin');

-- Subscription tiers
CREATE TYPE IF NOT EXISTS subscription_tier_enum AS ENUM ('basic', 'premium', 'enterprise');

-- Gender options
CREATE TYPE IF NOT EXISTS gender_enum AS ENUM ('male', 'female', 'other', 'prefer_not_to_say');

-- Assessment status
CREATE TYPE IF NOT EXISTS assessment_status_enum AS ENUM ('in_progress', 'completed', 'abandoned');

-- Question types for dynamic surveys
CREATE TYPE IF NOT EXISTS question_type_enum AS ENUM ('multiple_choice', 'text', 'number', 'boolean', 'scale', 'date');

-- Share methods for viral tracking
CREATE TYPE IF NOT EXISTS share_method_enum AS ENUM ('email', 'sms', 'social', 'direct_link');

-- Email template types
CREATE TYPE IF NOT EXISTS email_template_type_enum AS ENUM ('welcome', 'assessment_complete', 'report_share', 'campaign', 'reminder');

-- Campaign status
CREATE TYPE IF NOT EXISTS campaign_status_enum AS ENUM ('draft', 'scheduled', 'sending', 'sent', 'cancelled');

-- Subscription sources
CREATE TYPE IF NOT EXISTS subscription_source_enum AS ENUM ('website', 'assessment', 'referral', 'import');

-- Email delivery status
CREATE TYPE IF NOT EXISTS email_status_enum AS ENUM ('pending', 'sent', 'delivered', 'opened', 'clicked', 'bounced', 'failed');

-- Analytics metrics
CREATE TYPE IF NOT EXISTS analytics_metric_enum AS ENUM (
