-- Migration: 002_create_core_tables.sql (COMPLETE SAFE VERSION)
-- Description: Create core tables for the PHA platform (handles existing tables and missing columns)
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE table_name = 'user_profiles' AND column_name = 'practice_id'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_profiles ADD COLUMN role user_role_enum NOT NULL DEFAULT 'parent';
  END IF;
...
-- Add foreign key constraint for user_profiles.practice_id (only if not exists and column exists)
DO $$
BEGIN
  -- Check if practice_id column exists and constraint doesn't exist
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'practice_id'
  ) AND NOT EXISTS (
...
-- Add unique constraint for survey responses (only if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Add check constraint for email_sends (only if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'check_email_source'
  ) THEN
    ALTER TABLE email_sends 
    ADD CONSTRAINT check_email_source CHECK (
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_practice_id ON user_profiles(practice_id) WHERE practice_id IS NOT NULL;
